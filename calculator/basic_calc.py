...
class BasicCalculator(object):
    ...
    def __solve_expression(self, exp_arr, current_operators):
        ...
        while i < len(exp_arr):
            ...
            val = exp_arr[i + 1]
            if val == "(":
                sub_exp_length, val = self.__solve_expression_wrap(exp_arr[i + 2:])
    ...
    def __solve_expression_wrap(self, exp_arr):
        sub_exp_length, exp_arr = self.__solve_expression(exp_arr, ["*", "/"])
        _, exp_arr = self.__solve_expression(exp_arr, ["+", "-"])
